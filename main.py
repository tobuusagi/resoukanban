...
def task_weather_dashboard(device_config):
    ...
    # 🌟 侧边右侧黑色背景框 - 室内/室外/体感（与日程区域对齐）
    draw.rounded_rectangle([(260, 40), (385, 130)], radius=8, outline=0, fill=0)
    draw.text((268, 48), f"室内 {indoor['indoor_temp']} {indoor['indoor_humidity']}", font=font_small, fill=255)
    draw.text((268, 72), f"室外 {weather['humidity']}", font=font_small, fill=255)
    draw.text((268, 96), f"体感 {feel_temp}", font=font_small, fill=255)
